<div id="map-europe"> <!--   -->
    <ul class="europe"><!--   -->
        ...
        <?php
            ...
            $user_location = $criminal->getLocation();
            $current_location = new \Xe_GOC\inc\models\frontend\location($user_location);

            if(count($this->locations) > 0){
                foreach ($this->locations as $l){

	                $ticketprice = " - Huidige locatie";
	                $active_region = "";

                    if($l->getId() !== $user_location){ // CHECK IF NOT CURRENT COUNTRY OF USER
	                    $ticketprice = " &euro;".$current_location->getTicketPriceTo($l);
                    }
                    

	                ?>
                    <li class="eu<?php echo $l->getMapId(); ?>"><a href="?country=<?php echo $l->getId();?>"><?php echo $l->getName();  ?> <?php echo $ticketprice;  ?></a></li>
	                <?php
                }
        ...
        <!--
        <li class="eu1"><a href="#albania">Albania (€150)</a></li>
...
// CSSMap;
            let theMap = $("#map-europe").CSSMap({
                ...
                "responsive": "auto",
                "tapOnce": true,
                "activateOnLoad": ["<?php echo "eu".$current_location->getMapId(); ?>"]
            });
// END OF THE CSSMap;
