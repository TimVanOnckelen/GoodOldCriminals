...
class Crime {
	...
	function __construct($crime = -1,$u) {
		...
		// Set the user
		$this->user = new \Xe_GOC\Inc\Models\Frontend\CriminalUser($u);
		// get the last log
	...
	private function calculateCrime(){
		...
		// Car crime
		if($this->current_crime["crimecar"] == 1){
	...
	private function calculateRegularCrime(){
		...
		if($this->luck <= $this->current_crime["chance"]){
			...
			$this->calculateWin();
			// Show the result
			$this->result = $this->current_crime["success"]. __(' Je stal &euro;','xe_goc').$this->won_money;
			return true;
	...
	private function calculateCarCrime(){
		...
		if($this->luck <= $this->current_crime["chance"]){
			...
			// Calc win
			$car =  new Car();
			$car->doCarCrime($this->luck);
			$carObject = $car->getCarObject();

			if($carObject !== null && isset($carObject->post_title)){
				// Show the result
				$this->result = vsprintf(__('Het is gelukt om een %s te stelen met %s procent schade en een waarde van &euro; %s.','xe_goc'),array($carObject->post_title,$carObject->damage, $carObject->value));
				$this->result = $this->result.' '.__("De auto is toegevoegd aan je garage op je huidige locatie.","xe_goc");
				return true;
			}else{
	...
	public function canDoCrime(){
		...
		if(!isset($this->last_log["waittime"])){
			return  true;
		}

		// The end time
		$end_time = $this->last_log["last-try"] + $this->last_log["waittime"];
		...
		if($c_time >= $end_time)
			...
			$this->result = __('Je moet nog %s wachten om dit opnieuw te doen.');
		return false;
	...
	private function logLastCrime(){
		...
		$this->last_log["waittime"] = $this->current_crime["timetowait"];
