...
class Attack {
	 ...
	 * @var string
	 */
	public $result = 'Er ging iets mis, je was waarschijnlijk te traag.';
	...
	public function doAttack(){
			...
			// Defender on other location :)
			if($this->defender->getLocation() != $this->attacker->getLocation()){
				$defender_name = $this->defender->getName();
				$this->result = vsprintf(__( 'Je kan %s niet aanvallen. %s bevind zich op een andere locatie. Reis er naar toe om %s aan te vallen.', 'xe-goc' ),array($defender_name,$defender_name,$defender_name));
				return false;
			}

			// Cannot attack yourself
			if($this->defender->getId() == $this->attacker->getId()) {
				$this->result = __( 'Je kan jezelf niet aanvallen.', 'xe-goc' );
				return false;
			}

			// If user does not exsits, stop attack
			if($this->defender->userExsists() === false) {
				$this->result = __( 'Je kan niemand aanvallen die niet bestaat. Cheating??', 'xe-goc' );
				return false;
			}
			...
			// Set Xp
			$xp_t = new xpTransaction($xp,"add","attack");
			$xp_t->doTransaction();

			// Generate message
			$this->result = $this->messageGenerator();
	...
	private function timeCheck(){
		...
		if($c_time < $allowed_time){
			$this->result = sprintf(__('Je moet nog %d seconden wachten om %s opnieuw aan te vallen.'),$timeleft,$this->defender->getName());
			return false;
		}
	...
	private function maxAttackADay(){
		...
		if($count >= $this->max_attacks_a_day){
			$this->result = sprintf(__('Je hebt al %s aanvallen uitgevoerd vandaag. Morgen kan je weer aanvallen.'),$this->max_attacks_a_day);
			return false;
		}
	...
	private function messageGenerator(){

		if($this->defender_got_away === true){
			return vsprintf(__("%s verloor het gevecht van %s maar kon nog net optijd ontsnappen.","xe_goc"),array($this->loser->getName(),$this->winner->getName()));
		}

		return vsprintf(__("%s verloor het gevecht van %s. %s stal ook nog &euro; %s uit de portefeuille van %s.","xe_goc"),array($this->loser->getName(),$this->winner->getName(),$this->winner->getName(),$this->amount_won,$this->loser->getName()));
