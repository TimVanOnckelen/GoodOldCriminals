...
use Xe_GOC\Inc\Models\Frontend\Car;

class CarPostType {

	private $initiated = false;
	private $posttype = XE_GOC_POSTYPE_CARS_TYPE;
	private $post_id;
	...
	public function create_post_type() {
		...
		$labels = array(
			'name'               => _x( 'Car', 'post type general name', 'xe_goc' ),
			'singular_name'      => _x( 'Car item', 'post type singular name', 'xe_goc' ),
			'menu_name'          => _x( 'Car items', 'admin menu', 'xe_goc' ),
			'name_admin_bar'     => _x( 'Car item', 'add new on admin bar', 'xe_goc' ),
			'add_new'            => _x( 'Add new', 'Post', 'xe_goc' ),
			'add_new_item'       => __( 'Add new Car item', 'xe_goc' ),
			'new_item'           => __( 'New Car item', 'xe_goc' ),
			'edit_item'          => __( 'Edit Car item', 'xe_goc' ),
			'view_item'          => __( 'View Car item', 'xe_goc' ),
			'all_items'          => __( 'All Car items', 'xe_goc' ),
			'search_items'       => __( 'Search Car items', 'xe_goc' ),
			'parent_item_colon'  => __( 'Older Car items:', 'xe_goc' ),
			'not_found'          => __( 'No Car items found.', 'xe_goc' ),
			'not_found_in_trash' => __( 'No Car items in trash.', 'xe_goc'),
			'featured_image'     => __( 'Car image', 'xe_goc'),
	...
	public function custom_main_meta(){
		...
		require_once (XE_GOC_PLUGIN_PATH.'templates/backend/car-item/custom_meta.php');
	...
	public function saveMeta( $post_id, $post ){
		...
		foreach($_POST as $key => $item){ // loop every post item
			...
			$checkPost = strpos($key,Car::$save_needle);
			if($checkPost !== false){
