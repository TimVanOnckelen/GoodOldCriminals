...
use Xe_GOC\Inc\Models\Frontend\Company;

class CompanyPostType {

	private $initiated = false;
	private $posttype = XE_GOC_POSTYPE_COMPANY_TYPE;
	private $post_id;
	...
	public function create_post_type() {
		...
		$labels = array(
			'name'               => _x( 'Company item', 'post type general name', 'xe_goc' ),
			'singular_name'      => _x( 'Company item', 'post type singular name', 'xe_goc' ),
			'menu_name'          => _x( 'Company items', 'admin menu', 'xe_goc' ),
			'name_admin_bar'     => _x( 'Company item', 'add new on admin bar', 'xe_goc' ),
			'add_new'            => _x( 'Add new', 'Post', 'xe_goc' ),
			'add_new_item'       => __( 'Add new company item', 'xe_goc' ),
			'new_item'           => __( 'New company item', 'xe_goc' ),
			'edit_item'          => __( 'Edit company item', 'xe_goc' ),
			'view_item'          => __( 'View company item', 'xe_goc' ),
			'all_items'          => __( 'All companys', 'xe_goc' ),
			'search_items'       => __( 'Search companys', 'xe_goc' ),
			'parent_item_colon'  => __( 'Older companys:', 'xe_goc' ),
			'not_found'          => __( 'No companys found.', 'xe_goc' ),
			'not_found_in_trash' => __( 'No companys in trash.', 'xe_goc'),
			'featured_image'     => __( 'Company item image', 'xe_goc'),
	...
	public function custom_main_meta(){
		...
		require_once (XE_GOC_PLUGIN_PATH.'templates/backend/company-item/custom_meta.php');
	...
	public function saveMeta( $post_id, $post ){
		...
		foreach($_POST as $key => $item){ // loop every post item
			...
			$checkPost = strpos($key,Company::$save_needle);
			if($checkPost !== false){
