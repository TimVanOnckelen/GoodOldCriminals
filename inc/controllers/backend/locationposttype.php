...
use Xe_GOC\Inc\Models\Frontend\location;
use Xe_GOC\Inc\Models\Frontend\locationItem;

class LocationPostType {

    private $initiated = false;
    private $posttype = XE_GOC_POSTYPE_LOCATION_TYPE;
    private $post_id;
    ...
    public function create_post_type() {
        ...
        $labels = array(
            'name'               => _x( 'Location item', 'post type general name', 'xe_goc' ),
            'singular_name'      => _x( 'Location item', 'post type singular name', 'xe_goc' ),
            'menu_name'          => _x( 'Location items', 'admin menu', 'xe_goc' ),
            'name_admin_bar'     => _x( 'Location item', 'add new on admin bar', 'xe_goc' ),
            'add_new'            => _x( 'Add new', 'Post', 'xe_goc' ),
            'add_new_item'       => __( 'Add new location item', 'xe_goc' ),
            'new_item'           => __( 'New location item', 'xe_goc' ),
            'edit_item'          => __( 'Edit location item', 'xe_goc' ),
            'view_item'          => __( 'View location item', 'xe_goc' ),
            'all_items'          => __( 'All locations', 'xe_goc' ),
            'search_items'       => __( 'Search locations', 'xe_goc' ),
            'parent_item_colon'  => __( 'Older locations:', 'xe_goc' ),
            'not_found'          => __( 'No locations found.', 'xe_goc' ),
            'not_found_in_trash' => __( 'No locations in trash.', 'xe_goc'),
            'featured_image'     => __( 'Location item image', 'xe_goc'),
    ...
    public function custom_main_meta(){
        ...
        require_once (XE_GOC_PLUGIN_PATH.'templates/backend/location-item/custom_meta.php');
    ...
    public function saveMeta( $post_id, $post ){
        ...
        foreach($_POST as $key => $item){ // loop every post item
            ...
            $checkPost = strpos($key,location::$save_needle);
            if($checkPost !== false){
    ...
    public static function getAllLocations(){
    	...
    	$locations = get_posts(array(
    		"post_type" => XE_GOC_POSTYPE_LOCATION_TYPE,
		    "numberposts" => -1
	    ));
    	...
    	if(count($locations) > 0){
    		...
    		foreach ($locations as $l){
    			$return_locations[] = new location($l->ID);
		    }
	    }
